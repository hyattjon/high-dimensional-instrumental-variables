...
| `JIVE1`, `JIVE2` | Regress `Y` on the jackknife fit `X̃` (`X̃'X̃`) | **Stata `jive, jive1` / `jive2`**: coefficients agree to ~1e-8. Stata's *standard errors* differ from ours because of a bug in Stata's `jive.ado` (see below); ours use the correct residual. |
...
- **Standard errors:** `UJIVE1` and `UJIVE2` equal Stata's `robust` standard errors (~1e-8). For `JIVE1` and `JIVE2` ours differ from Stata's by 0.4-4%, and the reason is **a bug in Poi's `jive.ado`** (version 1.0.2, 17 Apr 2006), not in our code. Its `CalcUJIVE` routine defines a constant variable (`tempvar one`, line 295) and uses it to name the coefficient columns before computing the residual `Y − Xβ`, as its own comment says it intends to ("Use endogenous vars for residuals, not the predicted vars"). Its `CalcJIVE` routine uses the same name list (line 337) but never defines `one`, so the constant's coefficient is attached to the *last regressor* instead. Stata's `jive1`/`jive2` residual is therefore `Y − (β_t + β_cons)·T` with no controls, or with controls `Y − β_t T − … − (β_last + β_cons) W_last`, which is not a residual of the fitted model. Reproducing that residual in NumPy reproduces all 12 of Stata's `jive1`/`jive2` standard errors (default and robust, three data sets) to 1e-8 (`tests/test_stata_reference.py`), and the same routine's Root MSE and R-squared. Our standard errors use the intended residual `Y − Xβ`. The `ujive` variants are unaffected because that routine defines the constant.
- **Other reported numbers:** the first-stage F (with its degrees of freedom) equals Stata's for all four estimators on all three data sets. For `UJIVE1` and `UJIVE2` our Root MSE, R-squared and robust Wald F also equal Stata's on all three data sets. For `JIVE1` and `JIVE2` they differ from Stata's for the same reason as the standard errors (they are computed from the same buggy residual).
