...
def test_pandas_input_and_summary(capsys):
    ...
    res = UJIVE1(pd.Series(Y), pd.Series(x), pd.DataFrame(Z), W=pd.DataFrame(W))
    np.testing.assert_allclose(res.beta, UJIVE1(Y, x, Z, W=W).beta)
    res.summary()
    assert "UJIVE1 Regression Results" in capsys.readouterr().out
