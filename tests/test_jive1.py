...
def test_matches_explicit_projection_matrix(n, k, n_endog, n_controls):
    ...
    res = JIVE1(Y, x, Z, W=W)
    beta, robust_v, leverage = reference("JIVE1", Y, x, Z, W)
...
def test_pandas_input_and_summary(capsys):
    ...
    res = JIVE1(pd.Series(Y), pd.Series(x), pd.DataFrame(Z), W=pd.DataFrame(W))
    np.testing.assert_allclose(res.beta, JIVE1(Y, x, Z, W=W).beta)
    res.summary()
    assert "JIVE1 Regression Results" in capsys.readouterr().out
