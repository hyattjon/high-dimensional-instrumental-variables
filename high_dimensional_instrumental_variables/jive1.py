...
def JIVE1(Y: NDArray[np.float64], X: NDArray[np.float64], Z: NDArray[np.float64], W: NDArray[np.float64] | None = None, *, talk: bool = False) -> JIVE1Result:
    """
    Calculates the JIVE1 jackknife IV estimator: Y is regressed on the leave-one-out first-stage fitted values of the endogenous regressors (Stata jive naming; see the README section on naming).
