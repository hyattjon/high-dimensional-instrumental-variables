...
class JIVE2Result(IVResult):
    """Results of the JIVE2 estimator. See :class:`~high_dimensional_instrumental_variables._results.IVResult` for the attributes."""
...
def JIVE2(Y: NDArray[np.float64], X: NDArray[np.float64], Z: NDArray[np.float64], W: NDArray[np.float64] | None = None, *, talk: bool = False) -> JIVE2Result:
    """
    Calculates the JIVE2 jackknife IV estimator: Y is regressed on the leave-one-out first-stage fitted values of the endogenous regressors,
    beta = (X~'X~)^-1 X~'Y with X~_i = (Z_i pi_hat - h_i X_i) / (1 - 1/N).
    ...
    Returns:
        JIVE2Result: coefficients ``beta`` ordered [constant, endogenous regressors, controls], the robust covariance ``vcov`` and
    ...
    Notes:
        ...
        - Naming follows Stata's jive command (Poi 2006). The IV form of this estimator, beta = (X~'X)^-1 X~'Y, is UJIVE2 in this package. Coefficients equal Stata jive with jive2 (verified: see validation/); Stata's
          reported standard errors for jive2 differ from ours because of a bug in Stata's jive.ado (see the README section on naming).
    ...
    return JIVE2Result(**fit_jackknife_iv(Y, X, Z, W, leverage_denominator="n", iv_form=False, logger=logger, label="JIVE2"))
