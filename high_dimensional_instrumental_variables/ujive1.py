...
class UJIVE1Result(IVResult):
    """Results of the UJIVE1 estimator. See :class:`~high_dimensional_instrumental_variables._results.IVResult` for the attributes."""
...
def UJIVE1(Y: NDArray[np.float64], X: NDArray[np.float64], Z: NDArray[np.float64], W: NDArray[np.float64] | None = None, *, talk: bool = False) -> UJIVE1Result:
    """
    Calculates the UJIVE1 estimator: the Angrist, Imbens, and Krueger (1999) JIVE1 in IV form, beta = (X~'X)^-1 X~'Y, where the jackknife fit
    X~_i = (Z_i pi_hat - h_i X_i) / (1 - h_i) is used as the instrument for X.
    ...
    Returns:
        UJIVE1Result: coefficients ``beta`` ordered [constant, endogenous regressors, controls], the robust covariance ``vcov`` and
    ...
    return UJIVE1Result(**fit_jackknife_iv(Y, X, Z, W, leverage_denominator="leverage", iv_form=True, logger=logger, label="UJIVE1"))
